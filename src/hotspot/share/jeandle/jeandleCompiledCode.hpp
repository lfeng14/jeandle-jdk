...
#ifdef ASSERT
// The print helper function for print_deopt_value should have same ASSERT macro
// with the usage. So we keep them in jdk side instead of a method in
// DeoptValueEncoding.
static inline const char *
value_type_name(DeoptValueEncoding::DeoptValueType t) {
  switch (t) {
  case DeoptValueEncoding::LocalType:
    return "LocalType";
  case DeoptValueEncoding::StackType:
    return "StackType";
  case DeoptValueEncoding::ArgumentType:
    return "ArgumentType";
...
static inline void print_deopt_value(DeoptValueEncoding deopt_value) {
  ...
  tty->print_cr("DeoptValueEncoding: index: %d value_type: %s, basic_type: %s",
                deopt_value.index(), value_type_name(deopt_value.valueType()),
                type2name(static_cast<BasicType>(deopt_value.basicType())));
}
#endif
...
public:
  CallSiteInfo(
      JeandleCompiledCall::Type type, address target,
      bool is_method_handle_invoke = false,
      uint64_t statepoint_id = llvm::StatepointDirectives::DefaultStatepointID,
      Method *attached_method = nullptr, ciMethod *caller = nullptr,
      ciMethod *callee = nullptr, ciInstanceKlass *declared_holder = nullptr,
      int bci = -1, int bytecode = -1)
      : _type(type), _target(target),
        _is_method_handle_invoke(is_method_handle_invoke),
        _attached_method(attached_method), _statepoint_id(statepoint_id),
        _caller(caller), _callee(callee), _declared_holder(declared_holder),
        _bci(bci), _bytecode(bytecode) {
#ifdef ASSERT
    // We don't need to assign a unique statepoint id for each routine call
    // site, only call type and target is used.
    bool use_default_statepoint_id =
        (statepoint_id == llvm::StatepointDirectives::DefaultStatepointID);
    bool is_routine_call = (type == JeandleCompiledCall::ROUTINE_CALL);
    bool is_external_call = (type == JeandleCompiledCall::EXTERNAL_CALL);
    assert(use_default_statepoint_id == (is_routine_call || is_external_call),
           "routine calls and external calls should use the default statepoint "
           "id");
...
public:
  JeandleStackMap(int bci, ciMethod *method, OopMap *oop_map,
                  GrowableArray<ScopeValue *> *locals,
                  GrowableArray<ScopeValue *> *stack,
                  GrowableArray<MonitorValue *> *monitors, bool reexecute,
                  GrowableArray<ScopeValue *> *objects = nullptr)
      : _bci(bci), _method(method), _oop_map(oop_map), _locals(locals),
        _stack(stack), _monitors(monitors), _reexecute(reexecute),
        _objects(objects) {}
...
public:
  ...
  JeandleCompiledCode(ciEnv *env, ciMethod *method, int entry_bci)
      : _obj(nullptr), _elf(nullptr), _code_buffer("JeandleCompiledCode"),
        _routine_call_sites(), _non_routine_call_sites(), _const_sections(),
        _oop_handles(), _oop_handle_ids(), _oop_handle_info(), _offsets(),
        _exception_handler_table(), _implicit_exception_table(),
        _frame_size(-1), _prolog_length(-1), _env(env), _method(method),
        _routine_entry(nullptr), _func_name(JeandleFuncSig::root_method_name(
                                     _method, entry_bci != InvocationEntryBci)),
        _orig_pc_slot(nullptr), _orig_pc_offset_in_bytes(-1),
        _interpreter_frame_size_in_bytes(0), _has_method_handle_invoke(false) {}

  // For compiled Jeandle runtime stubs.
  JeandleCompiledCode(ciEnv *env, const char *func_name)
      : _obj(nullptr), _elf(nullptr), _code_buffer("JeandleCompiledStub"),
        _routine_call_sites(), _non_routine_call_sites(), _const_sections(),
        _oop_handles(), _oop_handle_ids(), _oop_handle_info(), _offsets(),
        _exception_handler_table(), _implicit_exception_table(),
        _frame_size(-1), _prolog_length(-1), _env(env), _method(nullptr),
        _routine_entry(nullptr), _func_name(func_name), _orig_pc_slot(nullptr),
        _orig_pc_offset_in_bytes(-1), _interpreter_frame_size_in_bytes(0),
        _has_method_handle_invoke(false) {}

  void install_obj(std::unique_ptr<ObjectBuffer> obj);

  void push_non_routine_call_site(CallSiteInfo *call_site) {
    _non_routine_call_sites.push_back(call_site);
  }
  ...
  int64_t duplicate_non_routine_call_site(uint64_t old_statepoint_id) {
    ...
    push_non_routine_call_site(new CallSiteInfo(
        old_call_site->type(), old_call_site->target(),
        old_call_site->is_method_handle_invoke(), new_statepoint_id,
        old_call_site->attached_method(), old_call_site->caller(),
        old_call_site->callee(), old_call_site->declared_holder(),
        old_call_site->bci(), old_call_site->bytecode()));
    return static_cast<int64_t>(new_statepoint_id);
  }
  llvm::SmallVector<CallSiteInfo *> &non_routine_call_sites() {
    return _non_routine_call_sites;
  }
  ...
  // insertion, so their keys stay valid for the life of this
  // JeandleCompiledCode (unlike the std::strings in _oop_handle_info's
  // SmallVector). Exposed so that the GetOopHandleName callback can return a
  // stable name pointer.
  ...
  JeandleExceptionHandlerTable *exception_handler_table() {
    return &_exception_handler_table;
  }

  ImplicitExceptionTable *implicit_exception_table() {
    return &_implicit_exception_table;
  }
  ...
  void update_interpreter_frame_size_in_bytes(int frame_size) {
    _interpreter_frame_size_in_bytes =
        MAX2(frame_size, _interpreter_frame_size_in_bytes);
  }
  int interpreter_frame_size_in_bytes() {
    return _interpreter_frame_size_in_bytes;
  }
...
private:
  ...
  // Note that the main difference between routine calls and non-routine calls
  // is that routine calls are found from relocation of compiled objects
  // directly, and non-routine calls are found from stackmaps and then matched
  // with the compile-time generated statepoint id.
  llvm::DenseMap<int, CallSiteInfo *>
      _routine_call_sites; // Contains all routine call sites, constructed from
                           // relocations of compiled objects in
                           // resolve_reloc_info.
  llvm::SmallVector<CallSiteInfo *>
      _non_routine_call_sites; // Contains all other call sites,
  ...
  address lookup_const_section(llvm::StringRef name,
                               JeandleAssembler &assembler);
  address resolve_const_reloc_site(LinkBlock &block, LinkEdge &edge,
                                   JeandleAssembler &assembler);
  address resolve_const_edge(LinkBlock &block, LinkEdge &edge,
                             JeandleAssembler &assembler);

  int parse_stackmap_prologue(
      StackMapParser::record_iterator &record,
      StackMapParser::RecordAccessor::location_iterator &location);
  JeandleStackMap *parse_stackmap(
      StackMapParser &stackmaps, StackMapParser::record_iterator &record,
      StackMapParser::RecordAccessor::location_iterator &location,
      int &num_deopts, const JeandleParseContext &parse_context,
      ciMethod *&next_inlinee, llvm::DenseMap<int, ObjectValue *> &vo_map,
      GrowableArray<JeandleDeferredVORefField> &deferred_voref_fields);
  LocationValue *
  new_location_value(const StackMapParser::LocationAccessor &location,
                     Location::Type type);
  void fill_one_scope_value(const StackMapParser &stackmaps,
                            const DeoptValueEncoding &encode,
                            const StackMapParser::LocationAccessor &location,
                            GrowableArray<ScopeValue *> *array);
  void fill_one_monitor_value(const StackMapParser &stackmaps,
                              const DeoptValueEncoding &encode,
...
public:
  ...
  static int32_t
  stack_offset(const StackMapParser::LocationAccessor &location) {
  ...
  static uint32_t
  getConstantUint(const StackMapParser &parser,
                  const StackMapParser::LocationAccessor &location);
  static uint64_t
  getConstantUlong(const StackMapParser &parser,
                   const StackMapParser::LocationAccessor &location);
  static float
  getConstantFloat(const StackMapParser &parser,
                   const StackMapParser::LocationAccessor &location);
  static double
  getConstantDouble(const StackMapParser &parser,
                    const StackMapParser::LocationAccessor &location);
};
